...
language.  This expression language has many of the usual operators,
such as \verb|+| and \verb|==|, from C-like languages, with the usual
semantics, and a few simple functions such as \verb|BadAddr| (which
tests whether a pointer can be safely dereferenced) and \verb|CondEq|
(which takes an x86 \verb|eflags| condition code word and checks
whether the Z flag is set, indicating that the values compared were
equal).  It also has forms for examining registers, such as \verb|rax|
or \verb|rsp|, and for loading values from memory.
...
the top of the diagram, the program issues the store
...
\caption{State machines produced for the example program.  Internal states are
  indicated by diamonds, terminal states by ovals, and edges by lines.}
...
sufficient to avoid the bug, but this is not the case.  Consider, for
...
would be sufficient to avoid the bug then one would have to conclude
that ensuring that this code only ran when \verb|foo| is false would
fix the bug.  This is unlikely to lead to a reasonable fix.

We determine the effects of unexecuted code using a simple static
analysis.  The first stage of this analysis is to build a fragment of
the program's static control flow graph starting from the branch
instruction and stopping when we encounter an instruction for which a
state machine has already been derived\footnote{We also impose a limit
  on the number of instructions examined, but this was not reached in
  any of our experiments.}.  We then eliminate any loops in the graph
using a scheme described in \S\ref{sect:loops} and so produce a DAG of
instructions rooted at the branch instruction and whose leaves are the
...
We must now construct appropriate code patches.  In this case, none of
the regions' control flow graphs will overlap, and so there will be
one patch for each.  For the single-instruction regions, the patch
will contain a lock operation, the instruction in the set, an unlock
operation, and a branch back to the original code.  For the load
region, the patch will contain the lock and unlock operations, the
...
\begin{itemize}
\item The only load instruction to be protected is \verb|G|.
\item There will be an unsafe region from \verb|Y| to \verb|V|. 
\item There will be single-instruction regions for \verb|V| and
  \verb|Y|.  Our prototype will recognise that these are redundant
  because they are covered by the unsafe region and eliminate them
  before proceeding any further.
\end{itemize}

The first region will be instantiated into a trivial
single-instruction patch, as the store instructions were in the
previous example.  We also construct a patch for the code in
figure~\ref{fig:broken_privatize}'s thread 2 which duplicates
instructions \verb|Y|, \verb|Z|, and \verb|V|, plus all of the
instructions between \verb|Y| and \verb|Z|, and which drops the lock
and branches back to instruction \verb|W| at the end.  These patches
would also fix the bug.  Furthermore, assuming that there are only a
small number of instructions between \verb|Y| and \verb|Z|, they will
be preferred by the prioritisation heuristic, because they require
...
shown in figure~\ref{fig:patch1}, before extending it to cover the
unsafe region and producing a final patch set shown in
figure~\ref{fig:patch2}.
...
although it may in some cases produce less efficient synchronisation
patterns.
