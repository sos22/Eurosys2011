...
but can make (limited) use of such information when it is available.
...
inlining every function into its called location, eliminating most
return instructions completely.  The remainder correspond to functions
which had started but not completed at the time of the crash, and we
assume that if any of those return then it indicates that the bug has
been avoided.
...
applying the patch to the program, either by patching a jump into the
entrypoint instruction or using the processor's debug facilities.
...
behavior of the program.  At present, only two properties of the
target program are assumed:

\begin{itemize}
\item Calling the \verb|__assert_fail| function is assumed to always
  indicate a bug.
\item Exiting due to an unhandled signal is likewise assumed to
  indicate a bug.  In particular, raising \verb|SIGSEGV| will be
  treated as a bug.
\end{itemize}

If the use-after-free analysis mentioned in section
...
assumptions will be false, but they appear to be reasonably rare.
...
to the behavior which a user might have encountered.  The race is
...
\caption{Time taken by the analysis phases of the thunderbird bug, in
  ...
  thread, in dynamic branches.}\editorial{Do I want to include some
  indication of how replay time changes as well?  Answer is pretty
  much that it doesn't, but might be worth saying so.}
...
\item The analysis performed by Kivati is somewhat simpler than SLI's,
  and so is likely to scale to more complicated bugs.
\end{itemize}
...
At the highest level, this approach combines the use of static
analysis, which considers many possible executions and produces
general fixes, and dynamic analysis, which considers only the observed
execution and produces much more targeted fixes.  We do not claim to
have found the optimal balance between these two approaches, or even
that a global optimum exists; better characterizing the trade-offs
involved is likely to suggest some useful improvements.
...
information compromising SLI's current ability to run on almost
arbitrary unmodified binaries.  We intend to investigate this idea
